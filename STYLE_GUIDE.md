...
- Run plugin actions and nodes through `PluginStore.run`, and add nodes through `PluginStore.addNode`, so confirmations, inputs, nodes, history and results behave the same from every menu, the inspector and the graph.
