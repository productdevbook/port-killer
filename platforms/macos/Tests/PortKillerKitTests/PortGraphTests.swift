...
struct PortGraphTests {
    ...
    var graph: PortGraph {
        PortGraph(
            ...
            consumers: [
                ...
                .init(node: GraphNode(id: "node:editor", kind: .pluginNode(editorNode), title: "Open in VS Code", subtitle: "", symbol: "chevron.left.forwardslash.chevron.right"), ports: [5173], origin: .link),
            ],
            idlePorts: [8080]
        )
    }
    ...
    @Test func focusesOnOneItemAndItsConnections() {
        ...
        #expect(node.nodes.map(\.id) == ["process:node", "port:3000", "port:9229", "favorites", "watch", "share", "quick:\(tunnelID)", "node:editor"])
    ...
    @Test func linkingAPortChangesItsSettings() throws {
        ...
        #expect(graph.change(linking: 9229, to: try #require(graph.node(id: "node:editor"))) == .connectPluginNode(editorNode, port: 9229))
        #expect(graph.change(linking: 5173, to: try #require(graph.node(id: "node:editor"))) == nil)
    ...
    @Test func unlinkingOnlyUndoesUserLinks() {
        ...
        #expect(graph.change(unlinking: GraphEdge(from: "port:5173", to: "node:editor", origin: .link)) == .disconnectPluginNode(editorNode, port: 5173))
    }
    ...
    @Test func nestsPortsInTheirProviders() {
        ...
        #expect(graph.blocks.map(\.id) == ["process:node", "process:vite", "port:8080", "favorites", "watch", "share", "quick:\(tunnelID)", "tunnel:dev", "node:editor"])
    }
